...
from unienv_interface.env_base.funcenv import (
    FuncEnv,
    ContextType,
    ObsType,
    ActType,
...
from unienv_interface.utils.framestack_queue import (
    FuncFrameStackQueue,
...
class FuncFrameStackWrapperState(Generic[StateType], StateClass):
    env_state: StateType
    obs_queue_state: Optional[FrameStackQueueState]
    action_queue_state: Optional[FrameStackQueueState]


class FuncFrameStackWrapper(
    FuncEnvWrapper[
        FuncFrameStackWrapperState[StateType],
        RenderStateType,
        BArrayType,
        ContextType,
        ObsType,
    ...
    def __init__(
        ...
        assert action_stack_size == 0 or action_default_value is not None, (
            "Action default value must be provided if action stack size is greater than 0"
        )
        assert obs_stack_size > 0 or action_stack_size > 0, (
            "At least one of observation stack size or action stack size must be greater than 0"
        )
        super().__init__(func_env)
        obs_is_dict = isinstance(func_env.observation_space, DictT)
        assert obs_is_dict or action_stack_size == 0, (
            "Action stack size must be 0 if observation space is not a DictSpace"
        )
        ...
        if action_stack_size > 0:
            self.action_deque = FuncFrameStackQueue(
        ...
        self.obs_deque = None
        if obs_stack_size > 0:
            self.obs_deque = FuncFrameStackQueue(
    ...
    def map_observation(
        self, state: FuncFrameStackWrapperState[StateType], observation: ObsType
    ) -> ObsType:
    ...
    def initial(self, *, seed=None, **kwargs):
        init_state, init_context, init_obs, init_info = self.func_env.initial(
            seed=seed, **kwargs
        )
    ...
    def reset(self, state, *args, seed=None, mask=None, **kwargs):
        env_state, context, observation, info = self.func_env.reset(
            state.env_state, *args, seed=seed, mask=mask, **kwargs
        )
        obs_queue_state = state.obs_queue_state
        action_queue_state = state.action_queue_state
        if self.obs_deque is not None:
            obs_queue_state = self.obs_deque.reset(
                obs_queue_state, initial_data=observation, mask=mask
            )
        if self.action_deque is not None:
            action_queue_state = self.action_deque.reset(
                action_queue_state, initial_data=self.action_default_value, mask=mask
            )
    ...
    def step(self, state, action):
        env_state, observation, reward, terminated, truncated, info = (
            self.func_env.step(state.env_state, action)
        )
        obs_queue_state = state.obs_queue_state
        action_queue_state = state.action_queue_state
        if self.obs_deque is not None:
        ...
        return (
            new_state,
            self.map_observation(new_state, observation),
            reward,
            terminated,
            truncated,
            info,
