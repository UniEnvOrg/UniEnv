...
from unienv_interface.env_base.env import (
    Env,
    ContextType,
    ObsType,
    ActType,
...
from unienv_interface.utils.framestack_queue import FrameStackQueue
...
class FrameStackWrapper(
    ...
    def __init__(
        ...
        if action_stack_size > 0:
            self.action_deque = FrameStackQueue(
                env.action_space,
                env.batch_size,
                action_stack_size,
            )
        ...
        self.obs_deque = None
        if obs_stack_size > 0:
            self.obs_deque = FrameStackQueue(
                env.observation_space, env.batch_size, obs_stack_size + 1
            )
            if action_stack_size > 0:
    ...
    def reverse_map_observation(
        ...
        if self.obs_deque is not None:
            obs_last = (
                sbu.get_at(self.obs_deque.output_space, stacked_obs, -1)
                if self.env.batch_size is None
                else sbu.get_at(
                    self.obs_deque.output_space, stacked_obs, (slice(None), -1)
                )
            )
            return obs_last
        else:
        ...
        if self.action_deque is not None:
            self.action_deque.reset(
                initial_data=sbu.get_at(  # Add a singleton batch dimension to the action
                    self.env.action_space, self.action_default_value, None
                )
                if self.env.batch_size is not None
                else self.action_default_value,
                mask=mask,
            )
        if self.obs_deque is not None:
            self.obs_deque.reset(initial_data=obs, mask=mask)
