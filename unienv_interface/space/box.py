...
class Box(Space[BArrayType, np.ndarray, BDeviceType, BDtypeType, BRNGType]):
    ...
    @property
    def bounded_below(self) -> BArrayType:
        bounded_below = -self.backend.array_api_namespace.inf < self._low
        return self.backend.array_api_namespace.broadcast_to(bounded_below, self.shape)

    @property
    def bounded_above(self) -> BArrayType:
        bounded_above = self.backend.array_api_namespace.inf > self._high
        return self.backend.array_api_namespace.broadcast_to(bounded_above, self.shape)
    ...
    def to_backend(self, backend : ComputeBackend, device : Optional[Any]) -> "Box":
        new_low = backend.from_other_backend(self._low, self.backend)
        new_high = backend.from_other_backend(self._high, self.backend)
        ...
        * :math:`(-\infty, \infty)` : normal distribution
        ...
        if self._dtype_is_float:
            ...
            unbounded = self.backend.array_api_namespace.logical_and(
                self.backend.array_api_namespace.logical_not(bounded_below), 
                self.backend.array_api_namespace.logical_not(bounded_above)
            )
            ...
            if self.backend.array_api_namespace.any(floating_point_error_idx):
                sample = self.backend.replace_inplace(
                    sample, floating_point_error_idx, self.backend.array_api_namespace.astype(high[floating_point_error_idx] - low[floating_point_error_idx] - 1, sample.dtype)
                )
