...
def _from_gym_space_dict(
    ...
    return Dict(
        backend=backend,
        spaces={key: from_gym_space(space, backend, dtype, device, seed) for key, space in gym_space.spaces.items()},
        device=device,
        seed=seed
    )
...
def _from_gym_space_union(
    ...
    return Union(
        backend=backend,
        spaces=[Space.from_gym_space(space, backend=backend, dtype=dtype, device=device) for space in gym_space.spaces],
        device=device,
        seed=seed
    )
