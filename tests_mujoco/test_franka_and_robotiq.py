...
EEF_SE3_GRIPPER_WORKSPACE = Box(
    ...
    low=np.concatenate(
        [EEF_SE3_WORKSPACE.low, np.array([0.0])],
    ),
    high=np.concatenate(
        [EEF_SE3_WORKSPACE.high, np.array([255.0])],
    ),
    dtype=np.float32
)
STEP_LIMIT = 1_000
...
def fr3_world() -> MujocoFuncWorld:
    ...
    _xml = os.path.join(_here, "assets", "franka_and_robotiq", "scene.xml")
...
def fr3_eef_actor(fr3_world : MujocoFuncWorld, fr3_actor : MujocoDefaultFuncActor) -> MujocoIKWrapper:
    ...
    return MujocoIKWrapper(
        ...
        fn_action_transform=lambda action, target_q: np.concatenate([target_q[:7], action[6:]]) if action is not None else np.concatenate([target_q[:7], [0.0]])
    )
...
def test_fr3_eef_interactive(
    ...
    for _ in range(STEP_LIMIT):
        ...
        real_target = np.concatenate([T_wt.translation(), T_wt.rotation().as_rpy_radians(), [0.0]])
