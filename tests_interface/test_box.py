...
def perform_single_space_cross_backend_device_test(space : Space, rng : typing.Any) -> None:
    ...
    for target_backend, target_devices in ALL_BACKENDS_AND_DEVICES.items():
        for target_device in target_devices:
            ...
            for _ in range(NUM_SAMPLES_TEST):
                rng, data = space.sample(rng)
                converted_data = converted_space.from_other_backend(data) if target_backend != space.backend else converted_space.from_same_backend(data)
                assert converted_space.contains(converted_data)
                converted_back_data = space.from_other_backend(converted_data) if target_backend != space.backend else space.from_same_backend(converted_data)
                assert space.contains(converted_back_data)
...
def perform_box_test(backend : typing.Type[ComputeBackend], device : typing.Optional[typing.Any], rng : typing.Any, np_rng : np.random.Generator):
    def generate_random_box_params(rng : typing.Any, np_rng : np.random.Generator):
        ...
        rng, values_1 = backend.random_exponential(rng, shape, lambd=1.0, dtype=dtype, device=device) if backend.dtype_is_real_floating(dtype) else backend.random_discrete_uniform(rng, shape, 0, 256, dtype=dtype, device=device)
        rng, values_2 = backend.random_exponential(rng, shape, lambd=1.0, dtype=dtype, device=device) if backend.dtype_is_real_floating(dtype) else backend.random_discrete_uniform(rng, shape, 0, 256, dtype=dtype, device=device)
        ...
        min_values = backend.replace_inplace(min_values, idx_min_unbound, -np.inf)
        max_values = backend.replace_inplace(max_values, idx_max_unbound, np.inf)
